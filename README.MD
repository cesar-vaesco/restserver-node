...
- 30 .- Cambiando las variables de entorno del proyecto en local a el repo de heroku
...
- 71.- Actualizar imagen de usuario y de producto
    - Al modelo de producto de le agrega el atributo img que va a permitir guardar una imagen, el modelo de usuarios ya contaba con ese atributo
    - En el archivo uploads.controller.js se agregaron los modelos Usuario y Producto, esto con la finalidad de hacer una busqueda para identificar si existen en la base de datos. De existir, realizar la actualización de la carga de la imagen
- 72.- Middleware validarArchivoSubir
    - Se crea un mediddleware donde se valida que al subir un archivo no vaya vacio
    - Se exporta a través del index de los middlewars
    - Se elimina de los controladores la función que anteriormente funcgía como validador de subir un archivo
    - En las rutas del router se agrega el validador como middleware
- 73.- Borrando archivos del servidor para evitar que existan dos o más imagenes guardadas del mismo producto o usuario
    - Se válida la existencia de la imagen y usando métodos de la lifreria fs(fileSystem), se elimina la imagen previa
- 74.- Servicio para mostrar las imagenes guardadas
    - Creación de ruta get para mostrar imagenes guardadas de Usuarios y Productos
- 75.- mostrar imagen de relleno
    - Se crea la ruta/path de la imagen que se va a cargar en caso de que el perfil del usuario y del producto no cuenten con una imagen prevía
    - Se regresa la imagen con la respuesta usando el método sendFile
 ...
 - *** Para usa Cloudinary se requiere contar con una cuenta ***

<div id="ambiando_variables_cloudinary"></div>
*** Modificando las variables de entorno configuradas en HEROKU ***

- 76.- Cargando imagenes a cloudinary
    - Se carga la variable de entorno de cloudinary (CLOUDINARY_URL )a el archivo .env la cual se encuantra en el Dashboard de la cuenta de cloudinary
    - Se crea una función llamada actualizarImagenCloudinary, la cúal contiene la lógica que va a permitir cargar las imagenes cloudinary
- 77.- Borrar imagenes en cloudinary
    - Se extrae el id con el que cloudinary guarda las imagenes para poder implementar la lógica que va a permitir borrar una imagen en caso de que el perfil ya cuente con alguna
- 78.- Estableciendo ```CLOUDINARY_URL``` como variable de entorno en HEROKU ---> * [verpaso30](#cambiando_variables_heroku) para identificar los pasos a seguir
    - Ejemplo:  heroku config:set CLOUDINARY_URL="cloudinary://URL" ---> * [verpaso76](#cambiando_variables_cloudinary) para identificar los pasos a seguir
    - Esto va a permitir poder usar el servicio de cloudinary y usar nustra app desde heroku de manera productiva
